...
function hbook_formValidation() {
	...
	jQuery('#btn-send-booking').click(function(e) {
		...
		if (isValid == 3 && firstValid == true && bookingExists == true) hbook_sendBooking("frn8945KdC");
	});
...
function hbook_confirmBooking(e, action) {
	...
	jQuery.ajax({
		...
		url: pluginsURL + '/hostel-booking/lib/confirm-order.php',
		type: 'post',
		data: {
			"booking_ref": ref,
			"action": action
